...
Process to build dataset
- Download CNNDM and XSum datasets into separate folders such as cnndm/ and xsum/.
...
Note that all following running script examples should be one line without backslash. Multiple lined examples here are for readability.

Annotate syntactic structures for train dataset
  Note: We use Stanford CoreNlp4.4.0 for that.
1. Download Stanford CoreNlp4.4.0 and unzip it.
2. Run CoreNlp4.4.0 server in the unzipped folder as follows,
...
- An example for XSum:
  Note: Our work requires two-phased fine-tuning for XSum. So, its build process has multiple iterations.
    - Build dataset (large sample set) without coreference for first phase fine-tuning:
        . annotated_dataset_builder.sh \
            --dataset_root "/dataset/root/common/to/all/rest/of/directories/or/folders" \
            --datasource xsum \
    ...
    - Reconcile struct feature vocabs from the two above built datasets.
      Note: the process recursively search from the annotated_data_dir to find the related structure feature vocabs and merge them into a consolidated one.
        . annotated_dataset_builder.sh \
            --dataset_root "/dataset/root/common/to/all/rest/of/directories/or/folders" \
            --datasource xsum \
            --downloaded_folder "." \
    ...
    - Final step: Convert structure feature labels into ids using the reconciled vocabularies.
...
Pickle datasets
  Note: Datasets are quite large. It is time-consuming to load them from json files every traing run. So, pickle them.
  An example (argument order is important):
    cd data_preprocess/pickle_dataset
    . pickle_dataset.sh \
        --dataset_root "/parent_directory/to/an_above_built_dataset" \
...
Build runtime validation and test datasets
    - Annotate dataset
      Note: As Stanford CoreNLP uses Penn Treebank for parsing structures. To ensure data distribution consistence with training dataset, we also annotate validation and test. But both validation and test dataset do not require extra structures. So, annotation process uses smaller set of feature process for both CNNDM and XSum.
        An example for builing CNNDM validation set,
        . corenlp_annotate.sh \
            --port              9000 \ # same port number as the running Stanford CoreNlp server.
            --datasource        cnndm \
            --dataset_root      /root/dataset/set/to/be/common/to/datasets/directories/or/folders \
            ...
            --output_dir        a_parent_folder/to/output_folder \
            --python_venv_dir   /your/python/venv/bin/directory
        Note: to annotate test set, just replace validation with test.
